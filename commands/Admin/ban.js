...
class Ban extends Command {
	...
	async run (message, args, data) {
		...
		if(!user){
			return message.error("Administration/ban:MISSING_MEMBER");
		}
		...
		if(banned.some((m) => m.user.id === user.id)){
			return message.error("Administration/ban:ALREADY_BANNED", {
				username: user.tag
			});
		}
		...
		if(!member.bannable) {
			return message.error("Administration/ban:MISSING_PERM");
		}
		await user.send(message.translate("Administration/ban:BANNED_DM", {
			username: user.tag,
		...
		message.guild.members.ban(user, { reason } ).then(() => {
			...
			message.sendT("Administration/ban:BANNED", {
				username: user.tag,
				server: message.guild.name,
			...
			if(data.guild.plugins.modlogs){
				...
				const embed = new Discord.MessageEmbed()
					.setAuthor(message.translate("Administration/ban:CASE", {
