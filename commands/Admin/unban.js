...
class Unban extends Command {
	...
	async run (message, args) {
		...
		if(!args[0]){
			return message.error("Administration/unban:MISSING_ID");
		}
		...
		if(isId){
			...
			user = this.client.users.cache.filter((u) => u.username === arr[0]).find((u) => u.discriminator === arr[1]);
		}
		...
		if(!banned.some((e) => e.user.id === user.id)){
			return message.success("Administration/unban:NOT_BANNED", {
				username: user.tag
			});
		}
		...
		message.success("Administration/unban:UNBANNED", {
			username: user.tag,
