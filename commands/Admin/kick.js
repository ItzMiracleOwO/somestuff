...
class Kick extends Command {
	...
	async run (message, args, data) {
		...
		if(!member){
			return message.error("Administration/kick:MISSING_MEMBER");
		}
		...
		if(!member.kickable) {
			return message.error("Administration/kick:MISSING_PERM");
		}

		await member.send(message.translate("Administration/kick:KICKED_DM", {
		...
		member.kick(reason).then(() => {
			...
			message.sendT("Administration/kick:KICKED", {
				username: member.user.tag,
				server: message.guild.name,
				moderator: message.author.tag,
				reason
			...
			if(data.guild.plugins.modlogs){
				...
				const embed = new Discord.MessageEmbed()
					.setAuthor(message.translate("Administration/kick:CASE", {
