...
module.exports = {
	...
	async init(client){
		...
		setInterval(async () => {
			Array.from(client.databaseCache.mutedUsers.values()).filter((m) => m.mute.endDate <= Date.now()).forEach(async (memberData) => {
				...
				if(member){
					guild.channels.cache.forEach(async channel => {
						 const permOverwrites = channel.permissionOverwrites.cache.get(member.user.id);
						 if(permOverwrites) permOverwrites.delete({});
					});
				}
				const user = member ? member.user : await client.users.fetch(memberData.id);
				const embed = new Discord.MessageEmbed()
					.setDescription(guild.translate("Administration/unmute:SUCCESS_CASE", {
						user: user.toString(),
						usertag: user.tag,
